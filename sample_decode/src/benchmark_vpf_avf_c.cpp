...
class Worker {
 ...
 public:
  ...
  bool decode_and_detect(size_t this_id) {
    ...
    FrameInfo frame;
    frame.totalSize = buffer.size();
    int rs = reader->getFrame(frame, buffer.data());
    if (rs == 2) {
    ...
    std::vector<uint8_t> packet_in(buffer.data(),
                                   buffer.data() + frame.frameSize);
    if (!parser->ParseSinglePacket(packet_in)) {
      return false;
    }
    if (!(parser->Width() > 0 && parser->Height())) {
    ...
    if (true) {
      ...
      if (!from_nv12_to_rgb_32f_planar) return false;
