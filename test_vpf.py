...
def test_vpf_to_rgb32f_planar(delete_session_folder, caplog):
    ...
    while True:
        ...
        logging.info(f"format: {rgb_32f_planar_surface.Format()}")
...
def test_vpf_to_rgb32f_planar_ffmpeg_demuxer(delete_session_folder, caplog):
    caplog.set_level(logging.INFO)
    logging.info("Start")
    input = os.path.join(os.path.dirname(__file__), "videos/1_2.mp4")
    gpu_id = 0
...
def test_rgb32f_planar_pitch(delete_session_folder, caplog):
    caplog.set_level(logging.INFO)
    s = nvc.Surface.Make(nvc.PixelFormat.RGB_32F_PLANAR, 416, 416, 0)
    logging.info(
